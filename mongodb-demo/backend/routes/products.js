...
router.get('/', (req, res, next) => {
  ...
  // }
  const products = [];
  // db.getDb().collection('products').find().sort({price: -1}).skip((queryPage - 1) * pageSize).limit(pageSize).forEach(productDoc => {
  db.getDb().collection('products').find().forEach(productDoc => {
...
router.get('/:id', (req, res, next) => {
  ...
  // res.json(product);
  db.getDb().collection('products').findOne({_id: new ObjectId(req.params.id)}).then(productDoc => {
    productDoc.price = productDoc.price.toString();
...
router.post('', (req, res, next) => {
  ...
  db.getDb().collection('products').insertOne(newProduct).then(result => {
        console.log(result);
...
router.patch('/:id', (req, res, next) => {
  ...
  db.getDb().collection('products').updateOne({_id: new ObjectId(req.params.id) }, {$set: updatedProduct}).then(result => {
...
router.delete('/:id', (req, res, next) => {
  db.getDb().collection('products').deleteOne({_id: new ObjectId(req.params.id)}).then(result => {
